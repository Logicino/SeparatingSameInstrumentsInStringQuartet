...
rootdir = '/Volumes/KinoLogic/Cadenza/data_enhance';
dir(rootdir)
...
while n <= length(Ensemblefilelist)
    ...
    % 记录这首歌里的分轨名称
    song_name = Ensemblefilelist(n).folder   % 分轨所在歌曲的歌曲文件夹
    stem_name_in_song = {}   % 记录每个分轨
    for m=1:Ninstruments
        stem_name_in_song{end+1} = Ensemblefilelist(m+n-1).name
    end
    stem_name_in_song = string(stem_name_in_song)
    % 将分轨的顺序随机化
    % 使用 randperm 生成一个随机排列的索引数组
    randomIndices = randperm(length(stem_name_in_song));
    % 使用这些索引来重新排列字符串数组
    stem_name_in_song = stem_name_in_song(randomIndices);

    for m=1:Ninstruments
        m
        stem_name = stem_name_in_song(m)
        [x1,fs1] = audioread(fullfile(song_name, stem_name));   % 加载音频
        if abs(fs1-fs)>1
    ...
    for m=1:Ninstruments                    %write audio files
        ...
        % fname = [rootout '/' Ensemblefilelist(m+n-1).name];    %file name
        % 存储文件名称
        stem_name = stem_name_in_song(m)
        fname = fullfile(rootout,sprintf('%d_%s', m, stem_name))
        ...
        I = strfind(Ensemblefilelist(n).folder,'/');    %make audio output name for mix
        fname = Ensemblefilelist(n).folder;
